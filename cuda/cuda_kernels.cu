...
void cuda_plot_graph_performance(float expiry, float strike, float cir_0, float x_0, float a, float k , float sigma, float rho,float r, char type, int* num_steps_array, int num_points, float exact_value){

    float output_2[3];
    float output_3[3];

    FILE *f = fopen("cuda/plot.dat", "w");
    if (f == NULL) {
    ...
    for (unsigned int i = 0; i < num_points; ++i){
        ...
        fprintf(f, "%f\t%f\t%f\t%f\t%f\n", 1./num_steps, output_2[0], output_2[0] - output_2[1], output_3[0],  output_3[0] - output_3[1]);
    };
    ...
    fprintf(f, "plot ");
    fprintf(f, "\"cuda/plot.dat\" using 1:2:3 with yerrorlines, \\\n");
    fprintf(f, "\"cuda/plot.dat\" using 1:4:5 with yerrorlines, \\\n");
    fprintf(f, "%f with lines lt 3", exact_value);
    fclose(f);
...
int main(void){
    ...
    num_steps_array[0] = 5;
    num_steps_array[1] = 10;
    num_steps_array[2] = 20;
    num_steps_array[3] = 30;
